...
<!-- 第一篇快速介绍 GojiJS，从各个角度吸引不同关注点的开发者 -->
...
<!--
后续几篇可以挑几个功能点详细讲：
1. lifecycle hooks：为什么要有universal hooks，原生框架的缺点GojiJS是怎么克服的，rendered effect的原理
2. CSS Module和CSS-in-JS：为什么需要抽出静态资源，CSS-in-JS如何实现
3. 分包加载：小程序加载机制、分包机制、独立分包机制，GojiJS是如何实现的
4. webpack：webpack编译config.ts编译实现原理，child compiler等，“WebPack在GojiJS中的使用”
主要目的：
1. 体现GojiJS的功能和特点（进阶用法）
2. 体现GojiJS的技术难点（深入原理）
 -->
...
<!-- 开篇就提供快速入门，满足希望立刻实践的用户，同时降低入门门槛 -->
...
<!-- 通过介绍组件和JSX，说明 GojiJS 是基于 React 的框架 -->
...
<!-- 通过一个例子体现 GojiJS 跨平台的意义，吸引有跨平台需求的开发者 -->

除了对 React 的良好支持，GojiJS 的另一个特点是对跨平台开发非常友好，在 Airbnb 公司里工程师们基于 GojiJS 开发了包括微信、支付宝、头条在内的五个小程序平台，支撑了XX级订单量。<!-- 这个数字需要PP review -->
